...
import type { McqQuestion, JsonRecord } from '@/types/app';
import type { SectionLimits } from '@/shared/utils/mockSectionLimits';
...
function titleCaseSubject(name: unknown): string {
  return String(name || '')
    .trim()
    .replace(/\s+/g, ' ')
    .split(' ')
...
function defaultMockLimitsForExam(examId: string) {
...
export function AdminWorkspace() {
  ...
  const [extraBankSubjects, setExtraBankSubjects] = useState<string[]>([]);
  const [tcsTopic, setTcsTopic] = useState('');
  const [qDraft, setQDraft] = useState<QDraft>(emptyDraft);
  const [bulkText, setBulkText] = useState('');
  const [queue, setQueue] = useState<McqQuestion[]>([]);
  const [tcsUploading, setTcsUploading] = useState(false);
  const [tcsMsg, setTcsMsg] = useState('');
  const [tcsErr, setTcsErr] = useState('');
  const [tcsUploadReport, setTcsUploadReport] = useState<{
    received?: number;
    inserted?: number;
    duplicates?: number;
    invalid?: number;
    bySubject?: Record<string, { inserted?: number; duplicates?: number; invalid?: number; received?: number }>;
  } | null>(null);
  const [tcsStats, setTcsStats] = useState<TcsStats | null>(null);
  const [statsLoading, setStatsLoading] = useState(true);
  const [instituteSummary, setInstituteSummary] = useState<InstituteSummary | null>(null);
  const [exporting, setExporting] = useState(false);

  const [mockLimits, setMockLimits] = useState<SectionLimits>({});
  ...
  const bankSubjects = useMemo(() => {
    const fromStats = (Array.isArray(tcsStats?.subjects) ? tcsStats.subjects : Object.keys(tcsStats?.bySubject || {})) as string[];
  ...
  const statsEntries = useMemo(() => {
    if (tcsStats?.bySubject && typeof tcsStats.bySubject === 'object') {
      return Object.entries(tcsStats.bySubject).sort((a, b) => Number(b[1]) - Number(a[1]));
    }
  ...
  const loadAllSubjects = async () => {
    try {
      ...
      const names = (Array.isArray(list) ? list : [])
        .map((s) => (typeof s === 'string' ? s : isRecord(s) ? String(s.name || '') : ''))
        .filter(Boolean);
  ...
  const loadTcsStats = async () => {
    ...
    try {
      ...
      setTcsStats(isRecord(res?.data) ? res.data as TcsStats : null);
    } catch {
      setTcsStats(null);
    } finally {
  ...
  const loadInstituteSummary = async () => {
    try {
      ...
      if (res?.status === 'success' && isRecord(res.data)) setInstituteSummary(res.data as InstituteSummary);
    } catch {
      /* ignore */
    }
  };

  const exportInstituteCsv = async (kind: 'mock' | 'syllabus') => {
    setExporting(true);
    try {
      ...
      showAppToast(errorMessage(e) || 'Export failed.', { variant: 'error' });
    } finally {
      setExporting(false);
    }
  };
  ...
  const persistMockLimits = (next: SectionLimits) => {
    setMockLimits(next);
    saveStoredLimitsForExam(examId, next);
  };

  const setMockLimitValue = (section: string, raw: string | number) => {
  ...
  const removeMockSection = (section: string) => {
  ...
  const ensureOfficialSubject = async (name: string) => {
    ...
    try {
      ...
      if (res?.status === 'success') {
        ...
        return { ok: true, created: !(isRecord(res.data) && res.data.alreadyExisted) };
      }
      return { ok: false, message: res?.message || 'Could not create subject.' };
    } catch (e) {
      return { ok: false, message: errorMessage(e) || 'Could not create subject.' };
    }
  };
  ...
  const addBulkToQueue = () => {
    ...
    setQueue((prev) => [...prev, ...items.map((item) => ({
      ...item,
  ...
  const handlePdfImport = (items: McqQuestion[], { errors = [], fileName = '' }: { errors?: string[]; fileName?: string } = {}) => {
    setTcsErr('');
    if (!items?.length) {
    ...
    try {
      ...
      if (data.bySubject && typeof data.bySubject === 'object') {
        setTcsUploadReport({
          ...
          received: Number(data.received ?? queue.length),
          bySubject: data.bySubject as Record<string, { inserted?: number; duplicates?: number; invalid?: number; received?: number }>,
        });
      }
      if (isRecord(data.stats)) setTcsStats(data.stats as TcsStats);
      else await loadTcsStats();
      if (Number(data.inserted ?? 0) > 0) setQueue([]);
    } catch (e) {
      setTcsErr(errorMessage(e) || 'Upload failed.');
    } finally {
      ...
      <div className="admin-exam-tabs">
        {EXAM_LIST.map((e) => (
          <button
            ...
            style={{ '--exam-accent': e.accent } as CSSProperties}
        ...
        <div className="admin-mock-pattern">
          ...
          <div className="admin-mock-pattern__add">
            ...
            <input
              ...
              onChange={(e) => setMockSectionCount(Number(e.target.value))}
