import { useState, useRef, useCallback, useEffect, type PointerEvent, type MouseEvent } from 'react';
import { createPortal } from 'react-dom';
import { StickyNote, X, GripVertical, Plus } from 'lucide-react';
import { StickyNotesPanel, type StickyNotesPanelHandle } from './StickyNotesPanel';
import { dailyStickyCount } from '../stickyNotesStorage';
import { loadFabPosition, saveFabPosition, clampFabPos, type FabPosition } from '../notesFloatingStorage';
import '../notes-floating-dock.css';

const DRAG_THRESHOLD = 2;

interface NotesFloatingDockProps {
  enabled?: boolean;
  openSignal?: number;
...
export function NotesFloatingDock({
  ...
  openSignal = 0,
  lockBodyScroll = true
}: NotesFloatingDockProps) {
  const [open, setOpen] = useState(false);
  const [pos, setPos] = useState<FabPosition>(loadFabPosition);
  const [noteCount, setNoteCount] = useState(() => dailyStickyCount());
  const [dragging, setDragging] = useState(false);
  const panelRef = useRef<StickyNotesPanelHandle>(null);
  ...
  const onFabPointerDown = useCallback((e: PointerEvent) => {
  ...
  const onFabPointerMove = useCallback((e: PointerEvent) => {
  ...
  const endFabPointer = useCallback((e: PointerEvent) => {
    const ds = dragState.current;
    if (!ds.active) return;
    if (ds.pointerId != null && e.pointerId !== ds.pointerId) return;
