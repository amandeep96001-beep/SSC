import { useState, useCallback, useEffect, useRef, lazy, Suspense, type FormEvent, type MouseEvent } from 'react';
...
function WorkspaceFallback() {
  return (
    <div className="app-loader-overlay app-loader-overlay--inline" aria-busy="true" aria-label="Loading">
...
export function Dashboard() {
  ...
  useGSAP(() => {
    if (workspaceRef.current && activeView !== 'test' && activeView !== 'results' && activeView !== 'mock_exam_active') {
      ...
      gsap.fromTo(workspaceRef.current,
        ...
        { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.48, ease: 'power3.out', clearProps: 'all' }
      );
      ...
      if (header) {
        gsap.fromTo(header,
          { opacity: 0, y: -14 },
          { opacity: 1, y: 0, duration: 0.46, ease: 'power3.out', clearProps: 'all' }
        );
      }
      if (cards.length > 0) {
        gsap.fromTo(cards,
          { opacity: 0, y: 24, scale: 0.985 },
          { opacity: 1, y: 0, scale: 1, duration: 0.5, stagger: 0.05, ease: 'power3.out', clearProps: 'all' }
        );
      }
    }
  ...
  useEffect(() => {
    ...
    const pull = async () => {
      try {
        ...
        await markNotificationsReadApi(rows.map((n) => n.id).filter((id): id is string => Boolean(id)));
      } catch {
  ...
  useEffect(() => {
    const goToView = (view: string, options: PathContext = {}) => {
  ...
  const loadVocabList = useCallback(async (page = 1, searchStr = '', categoryStr = 'All') => {
    ...
    try {
      ...
      if (res.status === 'success') {
        setVocabList(Array.isArray(res.data) ? res.data as VocabItem[] : []);
        const meta = isRecord(res.meta) ? res.meta : null;
        if (meta) {
  ...
  const openEditVocab = (item: VocabItem & { _id?: string }) => {
    setVocabForm({
      word: item.word || '',
      pos: item.pos || '',
  ...
  const handleVocabSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setVocabFormError('');
    setVocabFormSuccess('');
    const body = {
    ...
    try {
      ...
      setVocabFormError(errorMessage(err) || 'Failed to save. Please try again.');
    }
  };

  const handleVocabBulkSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setVocabBulkError('');
    setVocabBulkSuccess('');
    let parsedArray;
    try {
      ...
      setVocabBulkError('Invalid JSON format: ' + errorMessage(err));
      return;
    }
    ...
    try {
      ...
      if (res.status === 'success' || res.status === 'partial_success') {
        setVocabBulkSuccess(res.message || 'Imported.');
        await loadVocabList();
        setTimeout(() => {
      ...
      setVocabBulkError(errorMessage(err) || 'Failed to bulk import vocabulary.');
    }
  };
  ...
  const handleOpenEditModal = async (e: MouseEvent, topic: StudyTopic) => {
    e.stopPropagation();
    ...
    if (result.status === 'success' && isRecord(result.data)) {
      setEditTopicNotes(typeof result.data.notes === 'string' ? result.data.notes : '');
    } else {
      setEditTopicNotes('');
    }
  ...
  const handleEditTopicSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setTopicAddSuccess('');
    setTopicAddError('');
    ...
    if (res.success) {
      const report = isRecord(res.questionsReport) ? res.questionsReport : null;
      if (report) {
        setTopicAddSuccess(
  ...
  const handleDeleteClick = (e: MouseEvent, topicId: string) => {
    e.stopPropagation();
    setDeletingTopicId(topicId);
    setDeleteConfirmOpen(true);
  };

  const handleDeleteSubjectClick = (e: MouseEvent, subjectName: string) => {
    e.stopPropagation();
    setDeletingSubjectName(subjectName);
    setDeleteSubjectConfirmOpen(true);
  };

  const parseQuestionsBulk = (raw: string, setError: (msg: string) => void) => {
    if (!raw?.trim()) return [];
  ...
  const handleCreateTopicSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setTopicAddSuccess('');
    setTopicAddError('');
