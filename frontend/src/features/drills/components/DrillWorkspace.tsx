import { useState, useCallback, useEffect, type Dispatch, type SetStateAction, type MouseEvent, type ReactNode } from 'react';
...
function asRelatedList(value: unknown): RelatedQuestion[] {
  return Array.isArray(value) ? value as RelatedQuestion[] : [];
}

function vocabWordFromWrong(wq: WrongQuestion): string | null {
  if (wq?.word) return wq.word;
  const m = String(wq?.question || '').match(/"([^"]+)"/);
  return m ? m[1] : null;
}

const formatAIText = (text: string | null | undefined): ReactNode => {
  if (!text) return null;
  return text.split('\n').map((line, i) => {
...
function WrongQuestionCard({ wq, onRemove }: { wq: WrongQuestion; onRemove?: (question: string) => void }) {
  const [relatedQs, setRelatedQs]       = useState<RelatedQuestion[]>([]);
  ...
  const handleRelated = useCallback(async () => {
    ...
    try {
      ...
      const data = asRelatedList(res.data);
      setRelatedQs(data);
      if (data.length < 10) {
        setHasMore(false);
      }
  ...
  const handleShuffleRelated = useCallback(async (e?: MouseEvent) => {
  ...
  const handleAI = useCallback(async () => {
    ...
    try {
      ...
      setAiText((isRecord(res.data) && typeof res.data.explanation === 'string') ? res.data.explanation : 'No explanation returned.');
...
function RelatedQuestionRow({ rq, idx }: { rq: RelatedQuestion; idx: number }) {
...
interface DrillWorkspaceProps {
  ...
  maxTableBase: number;
  setMaxTableBase: Dispatch<SetStateAction<number>>;
  drillStats: DrillStats;
  drillFeedback: DrillFeedback;
  wrongQuestions: WrongQuestion[];
  clearWrongLog: () => void;
  removeWrongQuestion: (question: string | null | undefined) => void;
  clearWrongVocab: () => void;
  changeDrillType: (type: string) => void;
  submitDrillAnswer: (e?: { preventDefault?: () => void } | null, directAnswer?: string | null) => void;
  skipDrillQuestion: () => void;
...
export function DrillWorkspace({
  ...
  const switchDrill = (type: string) => { setActiveTab('drill'); changeDrillType(type); };
                    ...
                    <form onSubmit={submitDrillAnswer} className="submit-form">
                      {MCQ_TYPES.includes(drillType) && currentDrill.options ? (
                        ...
                        <div className="input-row">
                          <input type="text" placeholder={currentDrill.placeholder ?? undefined} value={userAnswer}
