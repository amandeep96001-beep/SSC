...
const CATEGORIES_BY_SUBJECT: Record<string, string[]> = {
  GK: [
    'History',
    'Ancient History',
    'Medieval History',
...
type AddQSuccess = (count: number, message: string) => void;

function FormMode({ onSuccess }: { onSuccess: AddQSuccess }) {
  const [form, setForm] = useState<AddQForm>(EMPTY_FORM);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const set = (key: keyof AddQForm) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => setForm((f) => ({ ...f, [key]: e.target.value }));
  ...
  const handleSubmit = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.status === 'success' || res.status === 'partial_success') {
        onSuccess(isRecord(res.data) ? Number(res.data.inserted || 0) : 0, res.message || '');
        setForm(EMPTY_FORM);
      } else {
        setError(res.message || 'Something went wrong.');
      }
    } catch (err) {
      setError(errorMessage(err) || 'Failed to add question. Please try again.');
    } finally {
      setLoading(false);
    }
        ...
        {['A', 'B', 'C', 'D'].map((letter) => (
          <div key={letter} className="add-q-field">
            ...
            <input
              ...
              value={form[(`opt${letter}`) as keyof AddQForm] as string}
              onChange={set((`opt${letter}`) as keyof AddQForm)}
              required
            />
          </div>
        ))}
...
function BatchMode({ onSuccess }: { onSuccess: AddQSuccess }) {
  ...
  const handleParse = useCallback(() => {
    ...
    try {
      ...
      const list = Array.isArray(json) ? json : (isRecord(json) && Array.isArray(json.questions) ? json.questions : null);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      setSubmitError(errorMessage(err) || 'Failed to submit. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [parsed, onSuccess]);
...
export function AddQuestionsPanel() {
  ...
  const [toast, setToast] = useState<{ count: number; message: string } | null>(null);

  const handleSuccess = useCallback((count: number, message: string) => {
