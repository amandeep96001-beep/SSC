...
import type { UseMockTestsReturn } from '../hooks/useMockTests';
...
export function MockWorkspace({ mockTestsApi, startMockExam, canEditPattern = false }: MockWorkspaceProps) {
  ...
  const [sectionLimits, setSectionLimits] = useState<SectionLimits>({});
  ...
  const persistLimits = (next: SectionLimits) => {
    setSectionLimits(next);
    saveStoredLimitsForExam(examId, next);
  };

  const setLimitValue = (section: string, raw: string | number) => {
            ...
            {mockTests.length === 0 && !loading ? (
              ...
              <div className="subjects-grid">
                ...
                {mockTests.map((test) => {
                  ...
                  return (
                  <div key={test._id} className="mock-glass-card">
                    <div className="mock-glass-card__body">
                      <div className="mock-glass-card__head">
                        <h3 className="mock-glass-card__title">{test.title}</h3>
                        <div className="mock-glass-card__actions">
                          <span className="mock-badge">{test.questionsCount || 0} Qs</span>
                          <button
                            type="button"
                            onClick={() => { setDeletingTestId(test._id || test.id || null); setDeleteConfirmOpen(true); }}
                      ...
                      <div className="mock-glass-card__meta">
                        {hasMeta ? (
                          <>
                            {test.year && <p>Year: {test.year}</p>}
                            {test.date && <p>Date: {test.date}</p>}
