...
const SSC_CGL_CHSL_CPO: ExamSyllabus = {
  id: 'ssc_cgl',
...
export const EXAM_PROFILE_TO_SYLLABUS: Record<string, string> = {
  ssc: 'ssc_cgl',
  banking: 'ssc_cgl',
  railways: 'railways',
  upsc: 'ssc_cgl',
...
export function getExamSyllabus(key: string = DEFAULT_EXAM_KEY): ExamSyllabus {
  return EXAM_SYLLABI[key] || EXAM_SYLLABI[DEFAULT_EXAM_KEY];
}

export function flattenTopics(syllabus: ExamSyllabus): FlattenedTopic[] {
  const list: FlattenedTopic[] = [];
...
export function countTopics(syllabus: ExamSyllabus): number {
