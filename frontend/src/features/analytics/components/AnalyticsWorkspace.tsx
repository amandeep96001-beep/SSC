...
export function AnalyticsWorkspace({ user }: { user: AppUser | null }) {
  ...
  const analyticsData = useMemo(() => {
    let totalMins = 0;
    const mockScores: Array<{ name: string; score?: number; accuracy?: number }> = [];
    ...
    progress.forEach((prog) => {
      ...
      const topicKey = prog.topicId || 'unknown';
      if (!topicDistribution[topicKey]) {
        topicDistribution[topicKey] = 0;
      }
      topicDistribution[topicKey] += 1;
    });
    ...
    const last7DaysMap: Record<string, { name: string; minutes: number }> = {};
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    const processDate = (prog: { timestamp?: string; elapsedTime?: string }) => {
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analyticsData.barData} margin={{ top: 8, right: 12, left: 4, bottom: 0 }}>
                  ...
                  <RechartsTooltip
                    ...
                    formatter={(value) => [`${Math.round(Number(value) || 0)} Mins`, 'Time Spent']}
