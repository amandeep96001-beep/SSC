...
function ScoreRing({ score = 0, maxScore, tone }: ScoreRingProps) {
...
function HistoryCard({ title, attempt, tone, score, maxScore, statusLabel, elapsed, timestamp, children, index }: HistoryCardProps) {
  return (
    <article
...
export function PerformanceWorkspace({ user }: { user: AppUser | null }) {
  ...
  const syllabusStats = useMemo(() => {
    ...
    const averageScore = progressCount > 0
      ? Math.round(progress.reduce((sum, curr) => sum + (curr.score || 0), 0) / progressCount)
      : 0;
  ...
  const mockStats = useMemo(() => {
    ...
    return {
      ...
      averageMockScore: mockCount > 0
        ? (mockProgress.reduce((sum, curr) => sum + (curr.score || 0), 0) / mockCount).toFixed(1)
        : '0.0',
      averageMax,
      averageMockAccuracy: mockCount > 0
        ? Math.round(mockProgress.reduce((sum, curr) => sum + (curr.accuracy || 0), 0) / mockCount)
        : 0,
      totalMockCorrect: mockProgress.reduce((sum, curr) => sum + (curr.correct || 0), 0),
      totalMockWrong: mockProgress.reduce((sum, curr) => sum + (curr.wrong || 0), 0)
    };
  }, [mockProgress, exam]);
              ...
              {mockProgress.length > 0 ? (
                <div className="perf-log-list">
                  {mockProgress.map((record, i) => {
                    const tone = (record.accuracy || 0) >= 80 ? 'green' : (record.accuracy || 0) >= 50 ? 'yellow' : 'red';
                    return (
                      <HistoryCard
                        key={`${record.mockTestId || record.title}-${record.attemptNumber || 1}-${i}`}
                        index={i}
                        title={record.title || 'Mock'}
                        attempt={record.attemptNumber || 1}
                        tone={tone}
                        score={record.score}
                      ...
                      >
                        ...
                        {isRecord(record.sectionTimes) && (
                          <div className="perf-section-times">
                            {Object.entries(record.sectionTimes as Record<string, number>).map(([sec, secs]) => (
                              <span key={sec} className="perf-section-chip">
                                {sec.slice(0, 3)} {Math.floor(Number(secs) / 60)}m {Number(secs) % 60}s
                              </span>
                            ))}
                          </div>
                        )}
