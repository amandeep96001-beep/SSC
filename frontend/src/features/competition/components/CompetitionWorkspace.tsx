...
export function CompetitionWorkspace({ user, setActiveView }: CompetitionWorkspaceProps) {
  ...
  const [resultData, setResultData]    = useState<CompetitionResult | null>(null);
  ...
  const userAnswersRef = useRef<(number | null | undefined)[]>([]);
  ...
  const handleAutoSkip = useCallback(() => {
    ...
    setAnswered(true);
    clearNextTimeout();
    nextTimeoutRef.current = window.setTimeout(() => moveToNext(), 1000);
  ...
  const handleAnswer = (optionIdx: number) => {
    ...
    setAnswered(true);
    clearNextTimeout();
    nextTimeoutRef.current = window.setTimeout(() => moveToNext(), 1200);
  };
