...
function tokenise(raw: string): McqToken[] {
  const tokens: McqToken[] = [];
 ...
 *  5. Newlines → <br/>
 */
function processPlainText(src: string): string {
...
function renderTextSegment(src: string): string {
  const tokens = tokenise(src);
  return tokens.map((tok) => {
    if (tok.type === 'math') {
    return renderKatex(tok.src, Boolean(tok.displayMode));
    }
 ...
 * 5. Join
 */
export function formatMcqHtml(raw: unknown): string {
