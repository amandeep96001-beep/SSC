import type { MockProgressRow, ProgressRow } from '@/types/app';
...
export function formatLastStudyLabel(iso?: string | null, now = new Date()): string {
  ...
  const dayDiff = Math.floor((startToday.getTime() - startOfD.getTime()) / 86_400_000);
  if (dayDiff < 7) {
