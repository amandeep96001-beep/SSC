...
type DiagramNode = BoxLabel | string;
...
function decodeBasicEntities(text: unknown): string {
  return String(text || '')
    .replace(/&nbsp;/gi, ' ')
    .replace(/&#160;/gi, ' ')
    .replace(/&amp;/gi, '&')
...
export function isLikelyHtml(content: unknown): boolean {
  if (!content) return false;
  return /<(?:p|h[1-6]|ul|ol|li|div|mark|br|strong|em|b|i|code|table|tr|td|th|span|blockquote|hr|pre|figure|section|article)\b/i.test(String(content));
}
...
function looksLikeAsciiDiagram(text: unknown): boolean {
...
function looksLikeTree(text: unknown): boolean {
  const lines = String(text || '').split('\n').filter((l) => l.trim());
  if (lines.length < 2) return false;
 ...
 */
function parseTreeLines(text: unknown): TreeNode[] {
  ...
  for (const rawLine of raw) {
    ...
    if (!prefixMatch) {
      ...
      const node: TreeNode = { label, depth, children: [] };
      if (depth === 0) {
        roots.push(node);
        stack.length = 0;
        stack.push(node);
...
export function treeToHtml(rawText: unknown): string | null {
...
function extractBoxLabels(text: unknown): BoxLabel[] {
  ...
  if (labels.some((l) => l._rowCells)) {
    const rows = labels
      .filter((l): l is BoxLabel & { _rowCells: string[] } => Boolean(l._rowCells))
      .map((l) => l._rowCells);
    const width = Math.max(...rows.map((r) => r.length));
    const nodes: BoxLabel[] = [];
  ...
  let current: BoxLabel | null = null;
  for (const line of lines) {
...
function extractArrowChain(text: unknown): string[] {
...
function renderSequenceDiagram(nodes: DiagramNode[]): string {
  if (!nodes.length) return '';
  ...
  nodes.forEach((node: DiagramNode, i: number) => {
    if (i > 0) parts.push('<span class="notes-diagram__arrow" aria-hidden="true">→</span>');
    const title = escapeHtml(typeof node === 'string' ? node : (node.title || node));
...
function renderFlowDiagram(nodes: DiagramNode[]): string {
  if (!nodes.length) return '';
...
export function asciiDiagramToHtml(rawText: unknown): string | null {
  const text = decodeBasicEntities(rawText).replace(/\r\n/g, '\n').trim();
...
function convertAsciiPreBlocks(html: string): string {
  return html.replace(/<pre\b[^>]*>\s*(?:<code\b[^>]*>)?([\s\S]*?)(?:<\/code>)?\s*<\/pre>/gi, (full: string, inner: string) => {
...
function convertLooseAsciiParagraphs(html: string): string {
  return html.replace(/(?:<p\b[^>]*>[\s\S]*?<\/p>\s*){2,}/gi, (block: string) => {
...
function parsePipeRow(line: string): string[] | null {
  ...
  let cols = trimmed.split('|').map((c: string) => c.trim());
...
export function markdownToHtml(markdown: unknown): string {
  ...
  let asciiBuf: string[] = [];
  let treeBuf: string[] = [];  // Buffer for tree-structure lines
  let bqBuf: string[] = [];    // Buffer for blockquotes
  ...
  const flushBq = () => {
    ...
    if (alertMatch) {
      ...
      const formattedBody = body.split('\n').map((line: string) => inlineMarkdown(line)).join('<br>');
  ...
  for (const rawLine of lines) {
    ...
    if (calloutMatch) {
      closeList();
      const label = calloutMatch[1].replace(/^\w/, (c: string) => c.toUpperCase());
...
function protectBlocks(html: string, pattern: RegExp, prefix = 'NOTES_BLOCK'): { html: string; blocks: string[]; prefix: string } {
  const blocks: string[] = [];
  const out = html.replace(pattern, (match: string) => {
...
function restoreBlocks(html: string, blocks: string[], prefix = 'NOTES_BLOCK'): string {
  const re = new RegExp(`__${prefix}_(\\d+)__`, 'g');
  return html.replace(re, (_: string, i: string) => blocks[Number(i)] || '');
}

function fixLatexFragments(html: string): string {
  return html
    ...
    .replace(/\$\\geq\$/gi, '≥')
    .replace(/\$\\%/g, '%')
    .replace(/\$([^\s$][^$]{0,250}[^\s$]|[^\s$])\$/g, (_: string, expr: string) => {
...
function cleanTableHtml(table: string): string {
  let cleaned = table
  ...
  cleaned = cleaned.replace(
    ...
    (row: string) => row.replace(/<\/?td\b/gi, (t: string) => t.replace('td', 'th'))
  );
...
function wrapTables(html: string): string {
  ...
  let out = html.replace(/<div class="notes-table-wrapper">\s*(<table\b[\s\S]*?<\/table>)\s*<\/div>/gi, (_: string, table: string) => {
    const token = `__NOTES_WRAPTABLE_${tables.length}__`;
    tables.push(cleanTableHtml(table));
    return token;
  });
  // Remaining bare tables
  out = out.replace(/<table\b[^>]*>[\s\S]*?<\/table>/gi, (table: string) => {
    const token = `__NOTES_WRAPTABLE_${tables.length}__`;
    tables.push(cleanTableHtml(table));
    return token;
  });
  return out.replace(/__NOTES_WRAPTABLE_(\d+)__/g, (_: string, i: string) => (
    `<div class="notes-table-wrapper">${tables[Number(i)] || ''}</div>`
  ));
}

function stripJunkAttributes(html: string): string {
  return html
    ...
    .replace(/\s+class\s*=\s*(["'])(?:(?!\1).)*?\1/gi, (match: string, q: string) => {
      if (/notes-/.test(match)) {
...
function stripChromeNoise(html: string): string {
  return html
...
function tidyStructure(html: string): string {
  return html
    .replace(/<p>\s*(?:&nbsp;|\s)*<\/p>/gi, '')
...
function promoteBoldHeadings(html: string): string {
  return html.replace(/<p\b[^>]*>\s*<(?:strong|b)\b[^>]*>([\s\S]*?)<\/(?:strong|b)>\s*<\/p>/gi, (full: string, inner: string) => {
...
function convertEmbeddedMarkdownIslands(html: string): string {
  // Paragraph that is purely a markdown heading
  html = html.replace(/<p\b[^>]*>\s*(#{1,4})\s+([\s\S]*?)<\/p>/gi, (_: string, hashes: string, body: string) => {
  ...
  html = html.replace(/(?:<p\b[^>]*>\s*\|[\s\S]*?\|\s*<\/p>\s*){2,}/gi, (block: string) => {
...
export function normalizeImportedHtml(raw: unknown): string {
  let html = String(raw || '');
...
function stripThemeBreakingStyles(html: string): string {
  return html
 ...
 */
export function prepareNotesHtml(content: unknown): string {
  if (!content) return '';
...
export function prepareNotesFromClipboard(html: string | null | undefined, plainText: string | null | undefined): string {
