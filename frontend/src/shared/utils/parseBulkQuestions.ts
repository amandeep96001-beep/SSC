...
const ANS_LETTER: Record<string, number> = { a: 0, b: 1, c: 2, d: 3, '1': 0, '2': 1, '3': 2, '4': 3 };
...
export interface BulkParseMeta {
  subject?: string;
  category?: string;
...
export function buildBulkMockExample(sections: string[] = MOCK_SECTIONS): string {
  const samples: Record<string, SampleQuestion> = {
    English: {
...
function splitBlocks(text: string): string[] {
  ...
  if (parts.length > 1) return parts.map((p: string) => p.trim()).filter(Boolean);
 ...
 */
export function normalizeSection(raw: unknown, allowedSections: string[] | null = null): string | null {
  ...
  if (allowed) {
    const exact = allowed.find((a: string) => a.toLowerCase() === s.toLowerCase());
  ...
  if (aliased) {
    ...
    const match = allowed.find((a: string) => a.toLowerCase() === aliased.toLowerCase());
...
function parseAnswerIndex(raw: unknown, options: string[]): number | null {
  ...
  const byText = options.findIndex((o: string) => o.toLowerCase() === lower);
...
function parseOneBlock(block: string, allowedSections: string[] | null): CompactMcq | null {
  const lines = block
    .split('\n')
    .map((l: string) => l.trim())
    .filter(Boolean);
...
export function parseBulkQuestions(raw: string, meta: BulkParseMeta = {}): {
  items: BulkMcqItem[];
  errors: string[];
  source: 'json' | 'text';
} {
  ...
  if (jsonList) {
    const items: BulkMcqItem[] = [];
    jsonList.forEach((rowUnknown, i) => {
      const row = isRecord(rowUnknown) ? rowUnknown : {};
      ...
      const section =
        ...
        || null;
      items.push({
        subject: typeof row.subject === 'string' ? row.subject : subject,
...
export function toCompactMcqs(
  items: BulkMcqItem[],
  { requireSection = false }: { requireSection?: boolean } = {}
): { items: CompactMcq[]; errors: string[] } {
  const out: CompactMcq[] = [];
  const errors: string[] = [];
  items.forEach((item: BulkMcqItem, i: number) => {
    ...
    const row: CompactMcq = {
      q: item.q,
      o: item.o.slice(0, 4).map((x: string) => String(x)),
      a: item.a,
      e: item.e || '',
    };
