...
async function fixTcs(db: MongoDb) {
...
async function fixTopicQuestions(db: MongoDb) {
...
async function fixMocks(db: MongoDb) {
  ...
  for (const m of mocks) {
    let changed = false;
    const questions = (m.questions || []).map((q: { q?: string; o?: string[]; e?: string; [key: string]: unknown }) => {
      const next = repairMcqFields({
        question: q.q,
        options: q.o,
        explanation: q.e,
