...
import type { ReminderRepeat } from './reminder.interface.js';
...
export class ReminderService {
  ...
  async markNotificationsRead(userId: string, body: Record<string, unknown>) {
  ...
  async testNotify(userId: string, userEmail: string | undefined) {
    const email = await resolveUserEmail(userId, userEmail);
