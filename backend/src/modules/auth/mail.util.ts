...
/** Absolute ceiling — API must never hang waiting on mail. */
const SMTP_SEND_HARD_MS = 12_000;
...
export async function sendMail({
  ...
  if (!smtpConfigured()) {
    ...
    console.info(`[mail] SMTP not configured — would send to ${recipient}: ${subject}`);
...
function buildOtpEmailHtml(code: unknown, { title, subtitle, lead }: { title: string; subtitle: string; lead: string }) {
...
export async function sendOtpEmail(
  ...
  try {
    ...
    console.error('[auth:otp] SMTP send failed:', errorMessage(err));
    if (!isHostedRuntime()) {
      console.info(`[auth:otp] Local fallback code for ${email}: ${code}`);
      return { sent: false };
    }
