...
export const getTcsStats: RequestHandler = async (req, res, next) => {
  try {
...
export const bulkUploadTcsQuestions: RequestHandler = async (req, res, next) => {
  try {
    ...
    const toInsert: NonNullable<ReturnType<typeof normalizeItem>>[] = [];
    let invalid = 0;
    let duplicates = 0;
    ...
    const guessSubject = (raw: unknown): string => {
 ...
 */
export const addQuestionsFromUser: RequestHandler = async (req, res, next) => {
  try {
    const payload = req.body;
    const list = Array.isArray(payload)
      ? payload
