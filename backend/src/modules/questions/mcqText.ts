...
export function stripCiteJunk(text: unknown): string {
  return String(text ?? '').replace(CITE_RE, '').trim();
}

function normalizeLatexFragments(text: unknown): string {
...
export function markupUnderlines(text: unknown): string {
  return String(text ?? '').replace(/(?<!_)__([^_\n](?:[\s\S]*?[^_\n])?)__(?!_)/g, '<u>$1</u>');
}

function convertLatexUnderlines(text: unknown): string {
...
export function normalizeMcqField(raw: unknown): string {
  if (raw == null) return '';
...
function wrapPhrase(body: string, phrase: string | null | undefined): string | null {
...
function closestSegment(body: string, option: string): string | null {
 ...
 */
export function autoUnderline(question: unknown, options: unknown[] = []): string {
  ...
  const finish = (wrappedBody: string) => `${instruction}\n${wrappedBody}`;
...
export function repairMcqFields({
  question,
  options,
  explanation,
}: {
